...
export const Cart = () => {
    ...
    const handlePayment = async () => {
        try {
            ...
            setMessage({title: 'Carrinho enviado para o checkout!', type: 'success'})
        } catch (error) {
    ...
    return (
        <>
                    ...
                    <TableBody>
                        {cart?.map((product) => (
                            <TableRow key={product.id}>
                                ...
                                <TableCell>
                                    <IconButton
                                        onClick={() => handleAddItemCart(product, false)}
                                    >
                                        -
                                    </IconButton>
                                    {product.quantity}
                                    <IconButton
                                        onClick={() => handleAddItemCart(product, true)}
                                    >
                                        +
                                    </IconButton>
                                </TableCell>
            ...
            <Box style={{ textAlign: "right", margin: "10px" }}>
                <Typography sx={{margin: '1rem 0'}}>Total em Reais: {totalPrice.toFixed(2)}</Typography>
